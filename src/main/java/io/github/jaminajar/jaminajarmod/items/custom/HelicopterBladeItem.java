...
public class HelicopterBladeItem extends SwordItem {
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        ...
        if (mainhand.getItem() == ModItems.HELI_BLADE && offhand.getItem() == ModItems.HELI_BLADE) {
            if (isUsable(mainhand) && isUsable(offhand)) {
                if (!user.isOnGround()) {
                    float f = user.getYaw();
                    ...
                    mainhand.damage(4- EnchantmentHelper.getLevel(Enchantments.UNBREAKING,mainhand), user, e -> e.sendEquipmentBreakStatus(
                            user.getActiveHand() == Hand.OFF_HAND ? EquipmentSlot.OFFHAND : EquipmentSlot.MAINHAND));
                    offhand.damage(4-EnchantmentHelper.getLevel(Enchantments.UNBREAKING,offhand), user, e -> e.sendEquipmentBreakStatus(
                            user.getActiveHand() == Hand.OFF_HAND ? EquipmentSlot.OFFHAND : EquipmentSlot.MAINHAND));
